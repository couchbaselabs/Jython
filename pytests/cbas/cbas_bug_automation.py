...
class CBASBugAutomation(CBASBaseTest):
    ...
    def test_data_partitions_with_default_data_paths(self):
        ...
        response = self.cbas_util.get_analytics_diagnostics(self.cbas_node)
        if not response:
            self.fail("Failed to get cbas diagnostics")
        io_devices = len(response["config"]["iodevices"])
        self.log.info("Number of IO devices on cluster %d" % io_devices)

        self.log.info("Fetch number of partitions")
        partitions = len(response["cc"]['partitions'])
